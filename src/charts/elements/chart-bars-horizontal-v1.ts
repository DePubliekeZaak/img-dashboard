    ...
    redraw() {
     ...
     //   let no_respondents = data.slice[0][this.ctrlr.graphObject.config.extra.columnForAverage]
        ...
        this.bars
            ...
            .transition()
            .duration(500)
            .attr("width", (d) => self.ctrlr.scales.x.scale(d.value));
        ...
        this.barLabels
            ...
            .attr('transform', function(d) {
                    ...
                    return 'translate(' + -self.ctrlr.config.padding.left + ',' +  (self.ctrlr.scales.y.scale(d.label) + (self.ctrlr.scales.y.scale.bandwidth() + offset)) + ')';
            })
            ;
        ...
        this.barValues
            .text(function(d) {
                ...
                if (d.no_respondents) {
                    let avg = (Math.round((10 * 100) * (d.value / d.no_respondents)) / 10).toString() + '%';
                    text = text + ' (' + avg + ')';
                }
            ...
            .attr('transform', function(d) {
                ...
                let offset = (window.innerWidth > breakpoints.bax) ? -(self.ctrlr.scales.y.scale.bandwidth() * .15) : 0;
                return 'translate(' + (self.ctrlr.scales.x.scale(d.value) + 6) + ',' +  (self.ctrlr.scales.y.scale(d.label) + (self.ctrlr.scales.y.scale.bandwidth() + offset)) + ')';
            })
            .attr('fill-opacity', 0)
            .transition()
            .delay(500)
