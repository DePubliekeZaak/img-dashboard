...
export class HtmlNumberSimple {
    ...
    draw() {
        ...
        number.style.fontSize = window.innerWidth < breakpoints.sm ? '1.6rem' : '2rem';
        number.style.lineHeight = "1.45";
        number.style.color = 'black';
        number.style.fontFamily = "Sora,sans-serif";
        number.style.fontWeight = "500";
        number.style.borderBottom =  '2px solid ' +  colours[this.parameter.colour][0];
    ...
    redraw(data,extraParameter) {
        ...
        if (this.parameter.format === "currency") {
            ...
            element.querySelector('.number.accented').innerText = convertToCurrency(data);
            ...
            let value =  (this.ctrlr.config.extra.decimal) ? Math.round(data * 10) / 10 : Math.round(data);
