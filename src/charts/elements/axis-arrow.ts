...
export class AxisArrow {
  ...
  draw() {
    ...
    this.ctrlr.svg.layers.axes
      ...
      .style(
        "font-size",
        window.innerWidth < breakpoints.xsm ? ".66rem" : ".85rem",
      )
  ...
  redraw() {
    ...
    this.ctrlr.svg.layers.axes
      .selectAll("path.arrow_" + this.axis)
      .attr("d", path)
    ...
    this.ctrlr.svg.layers.axes
      .selectAll("text.label_" + this.axis)
      .attr("x", direction === "horizontal" ? x - 8 : x - 2)
      .attr("y", direction === "horizontal" ? y + 16 : y - 30)
      .attr("fill", "black")
      .attr("text-anchor", (d) =>
        direction === "horizontal" || position === "right" ? "end" : "start",
      );
  }
}
