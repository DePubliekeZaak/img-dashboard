...
export class HtmlNumberTitled {
    ...
    draw() {
       ...
       // header.style.textAlign =  window.innerWidth < breakpoints.lg ? 'center' : 'left';
        ...
        topDiv.style.alignItems = window.innerWidth < breakpoints.lg ? 'center' : 'center';
        ...
        number.style.fontSize = window.innerWidth < breakpoints.sm ? '1.6rem' : '1.25rem';
        ...
        if(this.parameters[0].units) {
            ...
            units.style.marginBottom = '.37rem';
        ...
        bottomDiv.style.borderBottom =  '2px solid ' +  colours[this.parameters[0].colour][0];


        let _number = document.createElement('span');
        _number.classList.add('number');
        _number.classList.add('percentage');
        _number.style.fontSize = window.innerWidth < breakpoints.sm ? '1.6rem' : '2.4rem';
    ...
    redraw(data, extraParameter) {
        ...
        if (this.parameters[0].format === "currency") {
            ...
            element.querySelector('.number.accented').innerText = convertToCurrency(data[0]);

        } else if (this.parameters[0].format === "percentage") {
    
            let value =  Math.round(data[0] * 10) / 10;
            element.querySelector('.number.accented').innerText = value + "%";
        }
        
        else {
    
            let value =  (this.ctrlr.config.extra.decimal) ? Math.round(data[0] * 10) / 10 : Math.round(data[0]);
            element.querySelector('.number.accented').innerText = (value > 9999) ? thousands(value) : value;

            element.querySelector('.number.percentage').innerText = parseFloat(data[1]).toFixed(1) + "%";
        }
