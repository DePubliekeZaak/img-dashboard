...
import { convertToCurrency, shortenCurrency } from "../../shared/_helpers";
...
export default class MapLegend {
  ...
  draw(data: any[], yParameter: string) {
    ...
    bar.style.left =
      window.innerWidth > breakpoints.sm && window.innerWidth < breakpoints.bax
        ? "3rem"
        : "0rem";
    bar.style.top =
    ...
    if (
      this.ctrlr.config.extra.percentage ||
      this.ctrlr.parameters[0][0].format === "percentage"
    ) {
      max = max + "%";
    }
    ...
    const gradient =
      ...
      colours.lightBlue[3] +
      " 0%," +
      colours[this.ctrlr.parameters[0][0].colour][0] +
      " 100%)";
