...
export const relyOnCompleted = (filteredData: any[], tableParams: any, graphParams: any) => {
    ...
    for (let period of JSON.parse(JSON.stringify(filteredData))) {
        ...
        const startDate = new Date(start.getTime() + 24 * 60 * 60 * 1000).toLocaleDateString('nl-NL',{'dateStyle':'short'})

        if (i > 0 ) {
            row.push(
                startDate
                + ' t/m ' + 
                new Date(filteredData[i - 1]._einddatum).toLocaleDateString('nl-NL',{'dateStyle':'short'}) 
            ); 
        } else {
        ...
        period["_startdatum"] = new Date(start.getTime() + 24 * 60 * 60 * 1000).toLocaleDateString('nl-NL',{'dateStyle':'short'})
        if (i  > 0) {
            period["_einddatum"] = new Date(filteredData[i - 1]._einddatum).toLocaleDateString('nl-NL',{'dateStyle':'short'})
        }
...
export const fillEmptyMonths = (filteredData: any[], tableParams: any, graphParams: any):  { rows: any, dataByMonth: any } => { 
    ...
    for (let year of years) {
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            let m = filteredData.find( p => p._year == year && p._month == i.toString());                
            const row : string[] = [];
            ...
            if (m != undefined) {
                ...
                row.push(m._year);
                row.push(m._month);
                row.push(new Date(m._startdatum).toLocaleDateString('nl-NL',{'dateStyle':'short'}) + ' t/m ' + new Date(m._einddatum).toLocaleDateString('nl-NL',{'dateStyle':'short'})); 
