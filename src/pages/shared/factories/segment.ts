export const parseSegment = (
  ...
  if (group) {
    ...
    if (
      typeof group.periodization === "string" &&
      group.periodization.length > 0
    ) {
  ...
  if (graph) {
    ...
    if (
      typeof graph.periodization === "string" &&
      graph.periodization.length > 0
    ) {
  ...
  if (segment.gemeente !== "all") {
    const strippedKey = segment.key
      .replace("_cumul", "")
      .replace("_aantal", "");
    segment.key = segment.cumulative
      ? strippedKey + "_cumul"
      : strippedKey + "_aantal";
  } else {
