...
import { type Circle, Dimensions } from "@local/d3_types";
import { BaseType, SimulationLinkDatum, type SimulationNodeDatum } from "d3";
...
export class BallenbakSimulation {
  ...
  supply(data: any, groupName: string, groupCount?: number) {
    ...
    this.s.on("tick", (d: Circle) => {
      this.ctrlr[groupName].forceDirect();
    });
  }
