...
export default class HtmlLegendAsSumV2 {
    ...
    draw(data: PiePart[]) {
        ...
        this.ctrlr.parameters[1].forEach( (map: IParameterMapping, i: number) => {   
            tbody.appendChild(this.createRow(data[this.ctrlr.parameters[0].length + i], i, data, true));
        });
    ...
    createRow(map: PiePart, index: number, data: PiePart[], border) : HTMLDivElement {
