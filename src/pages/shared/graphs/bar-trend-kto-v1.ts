...
export class BarTrendKTOV1 extends core.GraphControllerV3 {
    ...
    html() {
        ...
        this.scrollingContainer.style.minWidth = (window.innerWidth < breakpoints.md) ? "600px" : "100%";
    ...
    prepareData(data: DataObject): DataObject {
        ...
        data.graphData = trimStart(data.graphData, this.parameters, 2);
        ...
        for (const pg of this.parameters) {
            ...
            for (const p of pg) {
                ...
                data[p.column] = createBars(p.column, p, data.graphData, this.segment) // .filter(b => b.value > 0)
            }
        }
    ...
    async redraw(data: any) {
        ...
        this.scales.y.set(data[this.segment.key].map(d => d.value < 0 ? 0 : d.value).concat([0, 10]));
