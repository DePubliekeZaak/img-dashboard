...
export class GroupControllerV1 implements IGroupCtrlr {
    ...
    prepareData(data: any) : any {
       ...
       // tableParams = tableParams.filter ( p => !p.column.includes("_cumulatief"))
        ...
        if (this.config.timeline !== undefined && this.config.timeline?.length > 0) {
            
            // @ts-ignore
            timeline = timelineList.filter((ti: Timeline) => this.config.timeline.indexOf(ti.label) > -1);
            timeline.sort((a, b) => Date.parse(a.date) - Date.parse(b.date));
        }

         // @ts-ignore
        const definitions = this.config.definitions && this.config.definitions?.length > 0 ?  definitionList.filter( d => this.config.definitions?.indexOf(d.name) > -1).sort() : [];


        return { 
            tableParams, graphParams, graphData, timeline, definitions, graphData_alt
        } 
    }
