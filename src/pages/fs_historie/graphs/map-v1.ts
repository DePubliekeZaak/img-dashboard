...
export class MapV1 extends core.GraphControllerV3 {
  ...
  html() {
    ...
    if (this.index === 4) {
      this.graphEl.style.margin =
        window.innerWidth < breakpoints.sm ? "0" : "0 auto 0 2.5rem";
    }
  }
  ...
  prepareData(data: DataObject): any {
    ...
    const param = data.graphParams[0];

    for (const yearData of data.grouped) {
