...
export class ProgressGroupV1 extends GroupControllerV1 { 
    ...
    prepareData(data: ImgData) : any {
        ...
        let { tableParams, graphParams, graphData, definitions, timeline, graphData_alt } = super.prepareData(data);

        graphData_alt = graphData_alt.filter ( p => p._yearmonth >'202410')
        graphData = graphData.filter( p => p._yearmonth >'202410')

        for (let period of data[dataGroup].filter( p => p._yearmonth >'202412')) {
