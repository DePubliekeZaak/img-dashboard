...
export class MakeupGroupTrendV1 extends GroupControllerV1 { 
    ...
    //    tableParams = tableParams.filter( p =>  p.column.includes("_cumulatief"));
        ...
        pre_headers: [{label: "", length: 3},{label: "Respondenten", length: 6}],
        headers:  ["Jaar","Maand","Maandcijfer","Totaal","FS","VES","WD","IMS","IMKJ"], // .concat(tableParams.map( p => p.label)), //  ["Betaalstroom"].concat(uniqueYears.map( y => y.toString())),
            rows
        };
