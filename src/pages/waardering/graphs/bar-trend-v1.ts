...
export class BarTrendV1 extends core.GraphControllerV3  {
    ...
    async init() {
        ...
        await super._init();
        if (this.scrollingContainer != null) await super._svg(this.scrollingContainer);
    ...
    prepareData(data: DataObject) : DataObject {
        ...
        let _data = (data.graphData_alt != undefined && this.group.config.endpoints.length == 2) 
        ? this.group.config.endpoints[1] != undefined && this.segment.periodization == "monthly" 
            ? data.graphData_alt 
            : data.graphData
        : data.graphData
        ...
        const createBars = (prop: string, param: IParameterMapping, data: KeyValue[]) => {
            ...
            for (let period of data) {
                ...
                if(this.parameters[1]) {
                    const no_respondents = parseInt(period[this.parameters[1][0].column].toString());
                    console.log(no_respondents);
                    ...
                    bs.push({
                        ...
                        meta: period,
                        value: period[prop] == null || no_respondents < 5 ? 0 : parseFloat(period[prop].toString()),
    ...
    async redraw(data: any) {
        ...
        this.scales.y.set(data[this.segment.key].map ( d => d.value > 0 ? d.value : 0).concat([0]));
