...
export class AOSBarTrendV1 extends core.GraphControllerV3  {
    ...
    async init() {
        ...
        for (let p of this.parameters[0]) {
            this.bars[p.column] = new elements.ChartBarTrend(this);
    ...
    async draw(data: DataObject) {
        ...
        for (let p of this.parameters[0]) {
            this.bars[p.column].draw(data[p.column])
    ...
    async redraw(data: any) {
        ...
        for (let p of this.parameters[0]) {
            this.bars[p.column].redraw(data[p.column])
        //    this.lines[p.column].redraw()
        }
